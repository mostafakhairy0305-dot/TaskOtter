...
func TestNodeInstallSkipsAlreadyInstalledVersion(t *testing.T) {
	...
	versionDir := filepath.Join(fnmRoot, "node-versions", "v18.0.0")
...
func fnmDryRunEnv(t *testing.T) []string {
	...
	// Pre-create the version directory used in dry-run specs so that the
	// node:use precondition ("is this version installed?") passes without a
	// real fnm installation.
	stubVersionDir := filepath.Join(fnmRoot, "node-versions", "v24.0.0")
	if err := os.MkdirAll(stubVersionDir, 0755); err != nil {
		t.Fatalf("failed to create stub version dir: %v", err)
	}
